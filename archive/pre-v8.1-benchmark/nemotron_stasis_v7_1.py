#!/usr/bin/env python3
# Nemotron + Dataset Probability Stasis v7.1
# Adds sentiment-aware weighting to entropy-stabilized FAISS field
# Color theme: pink gradient (#FF8AD8 → #FF1493)
...
# --- Embedding + sentiment model ---
model=SentenceTransformer('all-MiniLM-L6-v2')
sentiment=pipeline("sentiment-analysis", model="cardiffnlp/twitter-roberta-base-sentiment")
print("🔢 Encoding dataset (entropy + sentiment FAISS build)...")
...
def entropy_weight(similarities):
    p=np.clip(similarities,1e-8,1); p/=np.sum(p)
    ent=-np.sum(p*np.log(p))
    return 1/(1+ent)

def sentiment_weights(snips):
    outs=sentiment(snips)
    vals=[]
    for o in outs:
        if o['label']=='POSITIVE': vals.append(o['score'])
        elif o['label']=='NEGATIVE': vals.append(-o['score'])
        else: vals.append(0)
    return np.array(vals)

def highlight(txt,score):
    tokens=txt.split()
    return " ".join(f"{grad(score)}{t}{RESET}" for t in tokens)
...
def compute_stasis(query):
    ...
    sent_w=sentiment_weights(retrieved)
    sims_adj=sims*(1+sent_w)
    stability=1-np.std(sims_adj)*2.2
    entropy_w=entropy_weight(sims_adj)
    final_stability=max(0,min(1,stability*0.7+entropy_w*0.3))
    conf=np.mean(sims_adj)
    return final_stability,conf,retrieved,sims_adj

def generate_response(query,context,st):
    if st<STABILITY_THRESHOLD:
        return "The semantic field is unstable; more consistent data required before generation."
    ctx_txt="\n".join(context[:5])
    prompt=f"Context:\n{ctx_txt}\n\nQuestion: {query}\nAnswer clearly and briefly based only on this context."
    out=llm(prompt=prompt,max_tokens=180,temperature=0.72,stop=["</s>"])
    return out["choices"][0]["text"].strip()

def main():
    print(f"\n🤖 Nemotron + Dataset Probability Stasis v7.1 (Sentiment + Entropy + Safe Gen)\nType 'quit' to exit.\n")
    while True:
        q=input(f"{neon('💬 You: ')}")
        if q.strip().lower() in ['quit','exit']: break
        print(f"\n⚙️ Computing sentiment-weighted stasis field...\n")
